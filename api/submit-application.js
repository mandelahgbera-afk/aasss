...
async function handler(req, res) {
  ...
  try {
    ...
    if (files && Object.keys(files).length > 0) {
      console.log('[API] 📁 Processing file uploads...');
      
      for (const [fieldName, fileData] of Object.entries(files)) {
        try {
          // Validate file data
          if (!fileData || !fileData.data || !fileData.name) {
            console.warn(`[API] ⚠️  Invalid file data for ${fieldName}`);
            continue;
          }

          console.log(`[API] 📤 Uploading: ${fieldName} (${fileData.name})`);
          ...
          try {
            ...
            body.push(`--${boundary}`);
            body.push(`Content-Disposition: form-data; name="${paramName}"; filename="${fileData.name}"`);
            body.push(`Content-Type: ${fileData.type || 'application/octet-stream'}`);
            body.push('');
            
            // Build the body as buffer
            const textPart = body.join('\r\n') + '\r\n';
            const boundaryEnd = `\r\n--${boundary}--\r\n`;
            
            // Combine text and binary parts
            const textBuffer = Buffer.from(textPart, 'utf8');
            const endBuffer = Buffer.from(boundaryEnd, 'utf8');
            const fullBody = Buffer.concat([textBuffer, buffer, endBuffer]);
            ...
            if (!fileResponseData.ok) {
              ...
              console.log(`[API] File ${fieldName} (${fileData.name}) uploaded successfully`);
            }
          } catch (bufferErr) {
            console.error(`[API] File upload error for ${fieldName}:`, bufferErr.message);
          }
        } catch (err) {
          console.error(`[API] 💥 File upload error for ${fieldName}:`, err.message);
        }
      }
    }
